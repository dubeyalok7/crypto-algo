...
int checkmath(const unsigned char *hashvalue,
        ...
        const char *resultarray, int printResults, int printPassFail)
{
    ...
    if (printPassFail && resultarray) {
        int ret = checkmath(Message_Digest, resultarray, hashsize);
        ...
        int printPassFail)
{
    ...
    err = info ? hkdfReset(&hkdf, hashes[hashno].whichSha, keyarray, keylen):
        keyarray ? hmacReset(&hmac, hashes[hashno].whichSha, keyarray, keylen):
        USHAReset(&sha, hashes[hashno].whichSha);

    if(err != shaSuccess){
        fprintf(stderr, "hash(): %s Reset Error %d.\n",
    ...
    for(i = 0; i< repeatcount; ++i) {
        ...
        if(err != shaSuccess){
            fprintf(stderr, "hash(): %s Input Error %d.\n",
                    info ? "hkdf": keyarray ? "hmac": "sha", err);
            return err;
        }
    }
    ...
    if(numberExtrabits > 0) {
        ...
        if(err != shaSuccess){
            fprintf(stderr, "hash(): %s FinalBits Error %d.\n",
                    info ? "hkdf": keyarray ? "hmac": "sha", err);
            return err;
        }
    }
    ...
    if(err != shaSuccess){
        fprintf(stderr, "hash(): %s Result Error %d, counld not compute"
                "message_digest.\n",
                info ? "hkdf": keyarray ? "hmac": "sha", err);
        return err;
    }
        ...
        int printPassFail)
{
    ...
    sprintf(buff, "hkdfExpand %d", testno+1);
    printResult(okm, hkdfhashes[testno].okmlength,
            USHAHashName(hkdfhashes[testno].whichSha), "hkdf standard test",
            buff, hkdfhashes[testno].okmarray, printResults, printPassFail);
        ...
        int printResults, int printPassFail)
{
    ...
    if( err != shaSuccess) {
        fprintf(stderr, "hashfile(): %s Reset Error %d.\n",
                info ? "hkdf" : keyarray? "hmac" : "sha", err);
        return err;
    }
    ...
    if(skipSpaces) {
        ...
        while((nread = fread(buff, 1, sizeof(buff), hashfp)) > 0) {
            err = info ? hkdfInput(&hkdf, buff, nread):
                keyarray ? hmacInput(&hmac, buff, nread) :
                USHAInput(&sha, buff, nread);
            if (err != shaSuccess) {
